...
requiredMonFri = requiredMonFri - int16(sum(ismember(mTmpShiftDay.*int16(mShiftConstraint==2),[mondayArray fridayArray]),2));
...
while (~isempty(shiftStructStack))
    ...
    %% #22.03.29_10:40 This section decreases the number of branches to be visited by regularly ckecking some of the constraints (every 2nd day)
    passed = true;
    if mod(iDayPassed,2)==1
        
        tmpShiftIntMatrix = int16(shiftStructNode1.doctorMatrix==1);
        for iDoctor=1:numel(shiftStructNode1.requiredShiftArray)
            w=find(shiftStructNode1.doctorMatrix(iDoctor,:)==1);
            
            nFreeWeekend = 0;
            for iLongWeekendCellArray=1:length(longWeekendCellArray)
                if nnz(ismember(w,longWeekendCellArray{iLongWeekendCellArray}))==0
            ...
            nTueWenThuShift = nnz(ismember(w,tueWenThuArray)) - fixedTueWenThuShiftArray(iDoctor);
            if  requiredShiftArray(iDoctor) < requiredMonFri(iDoctor) + requiredWeekendDay(iDoctor) + nTueWenThuShift % minimum 1 shifts on monday or frnDayPassed
                passed=0;
                break
            end
            %
            nMonFriShift = nnz(ismember(w,[mondayArray fridayArray]));
            if nMonFriShift > maxMonFri(iDoctor) % minimum 1 shifts on monday or friday
                passed=0;
                break
            else
                passed=1;
...
%             
            nEngagedWeekendDay = nnz(ismember(w,[saturdayArray sundayArray]));
            if nEngagedWeekendDay > maxWeekendDay(iDoctor)
                passed=0;
                break
            else
                passed=1;
